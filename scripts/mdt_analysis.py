...
3. 分析（analysis）控制在 1-2 句，给出证据和方向。
4. 建议方向（suggested_direction）是可操作的具体建议。
5. **额外输出 3-5 条 consultation_questions**：下次问诊时患者应向医生确认的关键问题。要求：
   - 每条必须基于上面数据中的具体数值、趋势方向、基因检测结果或影像对比
   - 明确指出指标名+具体数值+变化方向+临床意义
   - 给出可操作的下一步建议（如是否需调整方案/加做检查/关注副作用/评估靶向适用性等）
   - 用患者能理解的自然中文，不用"AFP(+284%)"等数字按钮式表达
   - 示例：「CEA从14.65(2023-06基线)持续降至2.95(2025-12)，降幅80%，提示AG方案疗效显著。需向医生确认是否可考虑维持治疗或降级方案？」

输出 JSON，包含 concerns 数组和 consultation_questions 数组。"""
